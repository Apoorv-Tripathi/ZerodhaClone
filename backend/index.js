...
app.get("/allHoldings", async (req, res) => {
  try { res.json(await HoldingsModel.find({})); }
  catch { res.status(500).json({ error: "Failed to fetch holdings" }); }
});

app.get("/allPositions", async (req, res) => {
  try { res.json(await PositionsModel.find({})); }
  catch { res.status(500).json({ error: "Failed to fetch positions" }); }
});

app.get("/allOrders", async (req, res) => {
  try { res.json(await OrdersModel.find({}).sort({ _id: -1 })); }
...
app.post("/newOrder", async (req, res) => {
  try {
    ...
    if (!name || !qty || !mode) return res.status(400).json({ error: "Missing fields" });
    const order = await new OrdersModel({ name, qty, price, mode }).save();
    res.status(201).json({ message: "Order placed!", order });
  } catch { res.status(500).json({ error: "Failed to save order" }); }
});

// Seed endpoint — call once: GET /seed
